...
def move_robot_blue(data, id):
    ...
    if (data.kicker != 0 and data.dribbler):
        robot_command = RobotCommand(robot_id=id, move_command=move_command, kick_speed=data.kicker, kick_angle=0, dribbler_speed=DIBBLER_SPEED)
    elif (not data.kicker != 0 and data.dribbler):
        robot_command = RobotCommand(robot_id=id, move_command=move_command, kick_speed=0, kick_angle=0, dribbler_speed=DIBBLER_SPEED)
    elif (data.kicker != 0 and not data.dribbler):
        robot_command = RobotCommand(robot_id=id, move_command=move_command, kick_speed=data.kicker, kick_angle=0, dribbler_speed=0)
    else:
